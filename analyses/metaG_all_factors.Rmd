...
adonis_denit = adonis(denitRA_design[,-c(1:3)]~Plant*Treat, method = "bray", data = denitRA_design, perm=10000, set.seed=42)
adonis_denit

adonis_meth = adonis(methRA_design[,-c(1:3)]~Plant*Treat, method = "bray", data = methRA_design, perm=10000, set.seed=42)
adonis_meth

adonis_carb_metab = adonis(carb_metabRA_design[,-c(1:3)]~Plant*Treat, method = "bray", data = carb_metabRA_design, perm=10000, set.seed=42)
adonis_carb_metab

adonis_cytoCox = adonis(cytoCoxRA_design[,-c(1:3)]~Plant*Treat, method = "bray", data = cytoCoxRA_design, perm=10000, set.seed=42)
adonis_cytoCox

#PERMANOVA with History:
adonis_denit_H = adonis(denitRA_design[,-c(1:3)]~Plant*Hist + Plant*Treat, method = "bray", data = denitRA_design, perm=10000, set.seed=42)
adonis_denit_H

adonis_meth_H = adonis(methRA_design[,-c(1:3)]~Plant*Hist + Plant*Treat, method = "bray", data = methRA_design, perm=10000, set.seed=42)
adonis_meth_H

adonis_carb_metab_H = adonis(carb_metabRA_design[,-c(1:3)]~Plant*Hist + Plant*Treat, method = "bray", data = carb_metabRA_design, perm=10000, set.seed=42)
adonis_carb_metab_H

adonis_cytoCox_H = adonis(cytoCoxRA_design[,-c(1:3)]~Plant*Hist + Plant*Treat, method = "bray", data = cytoCoxRA_design, perm=10000, set.seed=42)
adonis_cytoCox_H
```
...
For Daniya, testing sub-setting of points:
```{r}
df1a_test <- df1a

#try removing the 'baseline' (_pre_pre) row, i.e., the first row:
df1a_test <- df1a_test[2:5,]

# Now use identical ggplot code from above to test:
plot_test <- ggplot(df1a_test, aes(x=V1, y=V2, shape = plant, colour=trt)) + theme_bw() + theme(panel.grid.major = element_blank(), 
               panel.grid.minor = element_blank(), 
               axis.line = element_line(colour = "black")) + 
  theme(panel.background = element_blank()) + 
  geom_errorbarh(aes(xmax=V1+V1e, xmin=V1-V1e), colour="black") + 
  ...
  scale_colour_manual(labels = c("Dry","Wet"), 
                      values = c("#997700", "#004488")) +
  scale_shape_manual(labels = c("No Plant","Plant"), values = c(5,9)) +
  theme(axis.title = element_text(size=14), 
        axis.text = element_text(size=14),
        axis.text.x = element_text(size=14), 
        panel.border = element_rect(colour = "black", size = 1.25)) + 
...
# Setup a 2-panel figure to confirm that the layouts are identical:
library(ggpubr) #load in library for multi-panel figures

two_panel_Daniya_test <- ggarrange(plot1a, plot_test, ncol = 2, nrow = 1, legend = "right", common.legend = T)

two_panel_Daniya_test
```
...
PCoA denitrification, all factors (including hydrologic history AS COLOR):
```{r PCoA: denitrification, all factors, HISTORY AS COLOR}
# Classical (Metric) Multidimensional Scaling; returns PCoA coordinates
# eig=TRUE returns eigenvalues; k = # of dimensions to calculate
pcoa_denit <- cmdscale(denitBC, k=3, eig=TRUE, add=FALSE)
...
explainvar1b_denit <- round(pcoa_denit$eig[1] / sum(pcoa_denit$eig), 3) * 100
explainvar2b_denit <- round(pcoa_denit$eig[2] / sum(pcoa_denit$eig), 3) * 100
sum.eiga <- sum(explainvar1b_denit, explainvar2b_denit)
...
pcoa_denitX.cent.dataframe.trts <- as.data.frame(pcoa_denit2.cent.dataframe) 
pcoa_denitX.cent.dataframe.trts$trt <- as.factor(trt_H)
pcoa_denitX.cent.dataframe.trts$plant <- as.factor(plant_H)
pcoa_denitX.cent.dataframe.trts$history <- as.factor(history)
pcoa_denitX.cent.dataframe.trts$trt_plant<- as.factor(trt_plant_combined)

#Run envfit to see which [GHG]s (start or finish) have good fit with ordination:
fit_denitX <- envfit(pcoa_denit$points, GHG_metaG, permutations = 10000)
fit_denitX

A_denitX <-as.list(fit_denitX$vectors)
vec_denitX <- as.data.frame(fit_denitX$vectors$arrows*sqrt(fit_denitX$vectors$r)*0.15)
p_denitX <- as.data.frame(A_denitX$pvals)
vec_denitX <- cbind(vec_denitX, p_denitX)
vec_denitX <- subset(vec_denitX, A_denitX$pvals<=0.1)

#Plot using ggplot2
dfXa <- as.data.frame(pcoa_denitX.cent.dataframe.trts)

plotXa <- ggplot(dfXa, aes(x=V1, y=V2, shape=trt_plant, colour=history)) + geom_errorbarh(aes(xmax=V1+V1e, xmin=V1-V1e, height=0.01), colour="black") +    
  geom_errorbar(aes(ymax=V2+V2e, ymin=V2-V2e, width=0.01), colour="black") +   
  geom_point(aes(shape = trt_plant, colour=history), size=5, stroke=2) +  
  theme_bw() +
  scale_color_manual(values=c("#997700", "#004488"), name="History") +
  scale_shape_manual(values = c(8, 2, 17, 1, 19), name="Treatment_Plant") +
  xlab("PCoA 1 (44.6%)") + ylab("PCoA 2 (25.1%)") + 
  geom_segment(data=vec_denit2, aes(x=0,xend=Dim1,y=0,yend=Dim2), size=1, arrow = arrow(length = unit(0.2, "cm")),colour="black",inherit.aes=F)+ 
 geom_text(data=vec_denit2, aes(x=Dim1, y=Dim2-0.001), label= expression(paste("mg CO"[2],"-C m"^{-2})), size=3, inherit.aes=F)+
  ggtitle("") + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
  theme(panel.background = element_blank()) + 
...
PCoA Methanogenesis, all factors, HISTORY AS COLOR:
```{r PCoA: Methanogenesis, all factors, HISTORY AS COLOR}
pcoa_meth <- cmdscale(methBC, k=3, eig=TRUE, add=FALSE)
...
pcoa_meth.groups <- paste(methRA_design$Treat, methRA_design$Plant, methRA_design$Hist, sep = "_")
pcoa_meth.points <- data.frame(pcoa_meth$points, group = pcoa_meth.groups)
...
pcoa_methX.cent.dataframe.trts <- as.data.frame(pcoa_meth.cent.dataframe) 
pcoa_methX.cent.dataframe.trts$trt <- as.factor(trt_H)
pcoa_methX.cent.dataframe.trts$plant <- as.factor(plant_H)
pcoa_methX.cent.dataframe.trts$history <- as.factor(history)
pcoa_methX.cent.dataframe.trts$trt_plant<- as.factor(trt_plant_combined)

#Run envfit to see which [GHG]s (start or finish) have good fit with ordination:
fit_meth <- envfit(pcoa_meth$points, GHG_metaG, permutations = 10000)
fit_meth

A_meth2 <-as.list(fit_meth$vectors)
vec_meth2 <- as.data.frame(fit_meth$vectors$arrows*sqrt(fit_meth$vectors$r)*0.15)
p_meth2 <- as.data.frame(A_meth2$pvals)
vec_meth2 <- cbind(vec_meth2, p_meth2)
vec_meth2 <- subset(vec_meth2, A_meth2$pvals<=0.1)

#Plot using ggplot2
dfXb <- as.data.frame(pcoa_methX.cent.dataframe.trts)

plotXb <- ggplot(dfXb, aes(x=V1, y=V2, shape=trt_plant, colour=history)) + geom_errorbarh(aes(xmax=V1+V1e, xmin=V1-V1e, height=0.01), colour="black") +    
  geom_errorbar(aes(ymax=V2+V2e, ymin=V2-V2e, width=0.01), colour="black") +   
  geom_point(aes(shape = trt_plant, colour=history), size=5, stroke=2) +  
  theme_bw() +
  scale_color_manual(values=c("#997700", "#004488"), name="History") +
  scale_shape_manual(values = c(8, 2, 17, 1, 19), name="Treatment_Plant") +
  xlab("PCoA 1 (41.3%)") + ylab("PCoA 2 (27.8%)") + 
  geom_segment(data=vec_meth2, aes(x=0,xend=Dim1,y=0,yend=Dim2), size=1, arrow = arrow(length = unit(0.2, "cm")),colour="black",inherit.aes=F)+ 
 geom_text(data=vec_meth2, aes(x=Dim1+0.02, y=Dim2-0.01), angle = -24, label= expression(paste("mg CH"[4],"-C m"^{-2})), size=3, inherit.aes=F)+
  ggtitle("") + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
  theme(panel.background = element_blank()) + 
  theme(axis.title = element_text(size=18), axis.text=element_text(size=18), 
...
```{r PCoA: Methanogenesis, all factors, HISTORY AS SHAPE}
pcoa_meth <- cmdscale(methBC, k=3, eig=TRUE, add=FALSE)
...
```{r PCoA: central carbohydrate metabolism, all factors, HISTORY AS SHAPE}
# Classical (Metric) Multidimensional Scaling; returns PCoA coordinates
# eig=TRUE returns eigenvalues; k = # of dimensions to calculate
pcoa_carb_metab <- cmdscale(carb_metabBC, k=3, eig=TRUE, add=FALSE)
...
```{r PCoA: cytochrome C oxidase, all factors, HISTORY AS SHAPE}
# Classical (Metric) Multidimensional Scaling; returns PCoA coordinates
# eig=TRUE returns eigenvalues; k = # of dimensions to calculate
pcoa_cytoCox <- cmdscale(cytoCoxBC, k=3, eig=TRUE, add=FALSE)
