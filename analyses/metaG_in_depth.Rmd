...
# Filter only Fe Reduction category: 
...
# Try to do all at once using a for loop:
...
start.Assim_SO4_red.dbplsr <- dbplsr(GHG_start_metaG_avg$mg.CO2.N.m.2 ~ as.matrix(Assim_SO4_redBC_start), ncomp = 7, method = "GCV")
summary(start.Assim_SO4_red.dbplsr)
#plot(start.Assim_SO4_red.dbplsr)

finish.Assim_SO4_red.dbplsr <- dbplsr(GHG_finish_metaG$mg.CO2.N.m.2 ~ as.matrix(Assim_SO4_redBC_finish), ncomp = 15, method = "GCV")
summary(finish.Assim_SO4_red.dbplsr)
#plot(finish.Assim_SO4_red.dbplsr)

# CH4 ~ Dissimilatory sulfate reduction:
start.Dissim_SO4_red.dbplsr <- dbplsr(GHG_start_metaG_avg$mg.CH4.N.m.2~ as.matrix(Dissim_SO4_redBC_start), ncomp = 7, method = "GCV")
summary(start.Dissim_SO4_red.dbplsr)
#plot(start.Dissim_SO4_red.dbplsr)

finish.Dissim_SO4_red.dbplsr <- dbplsr(GHG_finish_metaG$mg.CH4.N.m.2 ~ as.matrix(Dissim_SO4_redBC_finish), ncomp = 15, method = "GCV")
summary(finish.Dissim_SO4_red.dbplsr)
#plot(finish.Dissim_SO4_red.dbplsr)

# CH4 ~ Fe reduction:
start.FeReduction.dbplsr <- dbplsr(GHG_start_metaG_avg$mg.CH4.N.m.2 ~ as.matrix(FeReductionBC_start), ncomp = 7, method = "GCV")
summary(start.FeReduction.dbplsr)
#plot(start.FeReduction.dbplsr)

finish.FeReduction.dbplsr <- dbplsr(GHG_finish_metaG$mg.CH4.N.m.2 ~ as.matrix(FeReductionBC_finish), ncomp = 15, method = "GCV")
...
```{r PERMANOVAs on Finish Only gene sets}
# subset *RA_design to remove _pre samples


# Filter 'pre' samples out of *RA_design to get final samples only
#mtrBC_TIGRRA_FIN_design <- mtrBC_TIGRRA_design %>% dplyr::filter(Treat != "_pre" & Plant != "_pre")

# already done in PCoA code; moving this chunk below PCoAs.
...
#Run envfit with new Soil data (SSF = Soil Start & Finish):
fit_SO4_S_Assim_SSF <- envfit(pcoa_SO4_S_Assim$points, SoilGHG_StartFinish_Envfit, permutations = 10000)
fit_SO4_S_Assim_SSF
...
plot4a <- ggplot(df4a, aes(x=V1, y=V2, shape=history_plant, colour=trt)) + geom_errorbarh(aes(xmax=V1+V1e, xmin=V1-V1e, height=0.003), colour="black") +    
 ...
 geom_text(data=vec_SO4_S_Assim_SSF, aes(x=Dim1, y=Dim2-0.003), label= "%paint removed (redox)", size=3, inherit.aes=F)+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
  theme(panel.background = element_blank()) + 
  theme(axis.title = element_text(size=14), axis.text=element_text(size=14), 
          axis.text.x = element_text(size=14),  axis.text.y = element_text(size=14),
...
plot5a <- ggplot(df5a, aes(x=V1, y=V2, shape=history_plant, colour=trt)) + geom_errorbarh(aes(xmax=V1+V1e, xmin=V1-V1e, height=0.003), colour="black") +    
 ...
 geom_text(data=vec_SO4_S_Assim_SFO, aes(x=Dim1-0.007, y=Dim2+0.003), label=vec_SO4_S_Assim_SFO$labels, size=3.5, inherit.aes=F)+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
  theme(panel.background = element_blank()) + 
  theme(axis.title = element_text(size=14), axis.text=element_text(size=14), 
          axis.text.x = element_text(size=14),  axis.text.y = element_text(size=14),
...
# Add column with labels, for plotting:
vec_Dissim_SO4_red_SFO$labels <- c(expression(paste("mg CH"[4],"-C m"^{-2})), "P", "Cu")
...
#Run envfit with new Soil data (SSF = Soil Start & Finish):
fit_FeReduction_SSF <- envfit(pcoa_FeReduction$points, SoilGHG_StartFinish_Envfit, permutations = 10000)
fit_FeReduction_SSF
...
plot7a <- ggplot(df7a, aes(x=V1, y=V2, shape=history_plant, colour=trt)) + geom_errorbarh(aes(xmax=V1+V1e, xmin=V1-V1e, height=0.003), colour="black") +    
  ...
  guides(shape = guide_legend(override.aes = list(size = 4), order = 1), 
           colour = guide_legend(override.aes = list(pch=16, size = 4), order = 2)) +
    ggtitle("Fe Reduction Start/Finish")
...
#Run envfit with new Soil data (SFO = Soil Finish Only):
fit_FeReduction_SFO <- envfit(pcoa_FeReduction_FINAL$points, envfit_FINISH_numerical, permutations = 10000)
fit_FeReduction_SFO
...
# Add column with labels, for plotting:
vec_FeReduction_SFO$labels <- c(expression(paste("mg CH"[4],"-C m"^{-2})), "%C, %N", "%N, %C", "P", "Ca", "Cu", "CEC")
...
plot7b <- ggplot(df7b, aes(x=V1, y=V2, shape=history_plant, colour=trt)) + geom_errorbarh(aes(xmax=V1+V1e, xmin=V1-V1e, height=0.003), colour="black") +    
  ...
  geom_segment(data=vec_FeReduction_SFO, aes(x=0,xend=Dim1,y=0,yend=Dim2), linewidth=1, arrow = arrow(length = unit(0.2, "cm")),colour="black", alpha = 0.4,inherit.aes=F)+ 
 geom_text(data=vec_FeReduction_SFO, aes(x=Dim1, y=Dim2+0.02), label=vec_FeReduction_SFO$labels, size=3.5, check_overlap = T,inherit.aes=F)+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
  theme(panel.background = element_blank()) + 
  theme(axis.title = element_text(size=14), axis.text=element_text(size=14), 
          axis.text.x = element_text(size=14),  axis.text.y = element_text(size=14),
...
#Run envfit with new Soil data (SSF = Soil Start & Finish):
fit_SO4_red_combined_SSF <- envfit(pcoa_SO4_red_combined$points, SoilGHG_StartFinish_Envfit, permutations = 10000)
...
plot7c <- ggplot(df7c, aes(x=V1, y=V2, shape=history_plant, colour=trt)) + geom_errorbarh(aes(xmax=V1+V1e, xmin=V1-V1e, height=0.003), colour="black") +    
  ...
  geom_segment(data=vec_SO4_red_combined_SSF, aes(x=0,xend=Dim1,y=0,yend=Dim2), linewidth=1, arrow = arrow(length = unit(0.2, "cm")),colour="black", alpha = 0.4,inherit.aes=F)+ 
 geom_text(data=vec_SO4_red_combined_SSF, aes(x=Dim1, y=Dim2-0.004), label=vec_SO4_red_combined_SSF$labels, size=3.5, inherit.aes=F)+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
  theme(panel.background = element_blank()) + 
  theme(axis.title = element_text(size=14), axis.text=element_text(size=14), 
          axis.text.x = element_text(size=14),  axis.text.y = element_text(size=14),
  ...
  guides(shape = guide_legend(override.aes = list(size = 4), order = 1), 
           colour = guide_legend(override.aes = list(pch=16, size = 4), order = 2)) +
    ggtitle("Sulfate Reduction Start/Finish")
...
#Run envfit with new Soil data (SFO = Soil Finish Only):
fit_SO4_red_combined_SFO <- envfit(pcoa_SO4_red_combined_FINAL$points, envfit_FINISH_numerical, permutations = 10000)
...
# Add column with labels, for plotting:
vec_SO4_red_combined_SFO$labels <- c("Mg", "Ca", "S", "Mn", "Cu", "CEC")
...
plot7d <- ggplot(df7d, aes(x=V1, y=V2, shape=history_plant, colour=trt)) + geom_errorbarh(aes(xmax=V1+V1e, xmin=V1-V1e, height=0.003), colour="black") +    
  ...
  geom_segment(data=vec_SO4_red_combined_SFO, aes(x=0,xend=Dim1,y=0,yend=Dim2), linewidth=1, arrow = arrow(length = unit(0.2, "cm")),colour="black", alpha = 0.4,inherit.aes=F)+ 
 geom_text(data=vec_SO4_red_combined_SFO, aes(x=Dim1, y=Dim2+0.005), label=vec_SO4_red_combined_SFO$labels, size=3.5, inherit.aes=F)+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
  theme(panel.background = element_blank()) + 
  theme(axis.title = element_text(size=14), axis.text=element_text(size=14), 
          axis.text.x = element_text(size=14),  axis.text.y = element_text(size=14),
  ...
  guides(shape = guide_legend(override.aes = list(size = 4), order = 1), 
           colour = guide_legend(override.aes = list(pch=16, size = 4), order = 2)) +
    ggtitle("Sulfate Reduction Finish")
...
#ggsave("../figures/Thesis/FeRed_SO4Red_4panel.jpg", plot = FeRed_SO4Red_4panel, device=NULL, path=NULL, scale=1, height = 7, width = 10, limitsize = TRUE, dpi = 600)
```
...
# data frame name example: SO4_red_combinedRA_FIN_design
