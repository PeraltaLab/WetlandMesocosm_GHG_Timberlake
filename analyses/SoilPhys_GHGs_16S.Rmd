...
```{r flux input DFs with 2 point flux}
# Ambient to T30 is the reviewer recommended approach:
# Filter dataframes to only have time = 0 and time = 0.5
...
plot.CH4.flux.Trt <- ggplot(select_fluxes_combined_w_T0, aes(x=treatment, y=f0_CH4)) +
  ...
  theme(rect=element_rect(fill="transparent"))+
  theme(plot.background = element_rect(color=NA))+
  stat_pvalue_manual(data = CH4_MultComp.DF, comparisons = list(c("D", "I"), c("D", "W"), c("I", "W")), label = "p.SDCF.signif", size = 10, vjust = 1, hide.ns = T, y.position = max(select_fluxes_combined_w_T0$f0_CH4), step.group.by = "date_format", step.increase = 0.1)
