...
source("../bin/MothurTools.R") # having issues now, likely need to switch over to reshape2
...
for (unique_date in unique(ghg_flux_w_T0$date)) {
  for (unique_chamber in unique(ghg_flux_w_T0$chamber)) {
    ...
    new_row <- data.frame(
      ...
      co2_ppm = month_avg$co2_ppm,# FIX THESE NEXT
...
columns_to_fill <- c("ID", "V", "A") # Replace with your actual column names
