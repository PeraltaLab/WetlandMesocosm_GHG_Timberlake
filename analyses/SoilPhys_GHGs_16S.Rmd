...
ch4_flux.results <- gasfluxes(ch4_flux_input, method = c("linear","robust linear", "HMR"), plot = TRUE)

co2_flux.results <- gasfluxes(co2_flux_input, method = c("linear","robust linear", "HMR"), plot = TRUE)

n2o_flux.results <- gasfluxes(n2o_flux_input, method = c("linear","robust linear", "HMR"), plot = TRUE)
...
```{r ghg stats}
# Two-tailed T-test, is e.g., ch4_ppm different from ch4_ppm_avg?
t.test(ghg_ppm_filtered$ch4_ppm, ghg_ppm_filtered$ch4_ppm_avg, alternative = "two.sided", mu = 0)

t.test(ghg_ppm_filtered$co2_ppm, ghg_ppm_filtered$co2_ppm_avg, alternative = "two.sided", mu = 0)

t.test(ghg_ppm_filtered$n2o_ppm, ghg_ppm_filtered$n2o_ppm_avg, alternative = "two.sided", mu = 0)

# Paired:
t.test(ghg_ppm_filtered$ch4_ppm, ghg_ppm_filtered$ch4_ppm_avg, alternative = "two.sided", mu = 0, paired = T)

t.test(ghg_ppm_filtered$co2_ppm, ghg_ppm_filtered$co2_ppm_avg, alternative = "two.sided", mu = 0, paired = T)

t.test(ghg_ppm_filtered$n2o_ppm, ghg_ppm_filtered$n2o_ppm_avg, alternative = "two.sided", mu = 0, paired = T)

# Paired T test of plant vs no plant, compare results of T1 only to averaged (none of the inferences would be changed):
t.test(ch4_ppm ~ plant, data = ghg_ppm_filtered, alternative = "two.sided", mu = 0, paired = T)
t.test(ch4_ppm_avg ~ plant, data = ghg_ppm_filtered, alternative = "two.sided", mu = 0, paired = T)

t.test(co2_ppm ~ plant, data = ghg_ppm_filtered, alternative = "two.sided", mu = 0, paired = T)
t.test(co2_ppm_avg ~ plant, data = ghg_ppm_filtered, alternative = "two.sided", mu = 0, paired = T)

t.test(n2o_ppm ~ plant, data = ghg_ppm_filtered, alternative = "two.sided", mu = 0, paired = T)
t.test(n2o_ppm_avg ~ plant, data = ghg_ppm_filtered, alternative = "two.sided", mu = 0, paired = T)
```
...
# Individual identifiers (wid) must be factor:
...
```{r three-way repeated measure ANOVA}
# Set factors:
ghg_ppm_filtered$plant <- as.factor(ghg_ppm_filtered$plant)
ghg_ppm_filtered$treatment <- as.factor(ghg_ppm_filtered$treatment)
ghg_ppm_filtered$history <- as.factor(ghg_ppm_filtered$history)

# 4-way (?) ANOVAs:
  # CH4:
fourwayAOV_CH4 <- anova_test(
  data = ghg_ppm_filtered, dv = ch4_ppm, wid = chamber,
  between = c(plant, treatment, history),
  within = date_short
  )
get_anova_table(fourwayAOV_CH4)

fourwayAOV_CH4_AVG <- anova_test(
  data = ghg_ppm_filtered, dv = ch4_ppm_avg, wid = chamber,
  between = c(plant, treatment, history),
  within = date_short
  )
get_anova_table(fourwayAOV_CH4_AVG) # Averaging does not change statistical inferences

  # CO2:
fourwayAOV_CO2 <- anova_test(
  data = ghg_ppm_filtered, dv = co2_ppm, wid = chamber,
  between = c(plant, treatment, history),
  within = date_short
  )
get_anova_table(fourwayAOV_CO2)

fourwayAOV_CO2_AVG <- anova_test(
  data = ghg_ppm_filtered, dv = co2_ppm_avg, wid = chamber,
  between = c(plant, treatment, history),
  within = date_short
  )
get_anova_table(fourwayAOV_CO2_AVG) # Averaging DOES change statistical inferences

  # N2O:
fourwayAOV_N2O <- anova_test(
  data = ghg_ppm_filtered, dv = n2o_ppm, wid = chamber,
  between = c(plant, treatment, history),
  within = date_short
  )
get_anova_table(fourwayAOV_N2O)

fourwayAOV_N2O_AVG <- anova_test(
  data = ghg_ppm_filtered, dv = n2o_ppm_avg, wid = chamber,
  between = c(plant, treatment, history),
  within = date_short
  )
get_anova_table(fourwayAOV_N2O_AVG) # Averaging does not change statistical inferences
```
