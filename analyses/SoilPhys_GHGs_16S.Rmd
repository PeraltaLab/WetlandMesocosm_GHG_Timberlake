...
```{r Combined GHG Boxplot}
CombPlot_flux_NPvP <- ggarrange(plot.CH4.flux.NPvP, plot.CO2.flux.NPvP, plot.N2O.flux.NPvP, labels = c("A", "B", "C"), ncol = 1, nrow = 3, legend = "none")
...
# Add on SDCFlig p-values:
CH4_MultComp.DF$SDCF_p <- c(1, 1, 1, 1, 1, 1, 1, 1, 1, SDCFlig_CH4_7.25$p.val, 1, 1, 1)
...
plot.CH4.flux.Trt <- ggplot(select_fluxes_combined_w_T0, aes(x=treatment, y=f0_CH4)) +
  ...
  theme(rect=element_rect(fill="transparent"))+
  theme(plot.background = element_rect(color=NA))+
  stat_pvalue_manual(data = CH4_MultComp.DF, comparisons = list(c("D", "I"), c("D", "W"), c("I", "W")), label = "p.SDCF.signif", size = 10, vjust = 1, hide.ns = T, y.position = (max(select_fluxes_combined_w_T0$f0_CH4)+200), step.group.by = "date_format", step.increase = 0.1)
...
```{r CO2 T30 ppm Treatment}
# Kruskal-Wallis test on Treatment, and multiple comparisons:
CO2_kruskal<- compare_means(co2_ppm~treatment, select_fluxes_combined_w_T0, group.by="date_format", method = "kruskal.test")
CO2_multipleComparisons<- compare_means(co2_ppm~treatment, select_fluxes_combined_w_T0, group.by="date_format", method = "wilcox.test", p.adjust.method = "fdr")
...
# Plotting:
plot.CO2.T30ppm.Trt <- ggplot(select_fluxes_combined_w_T0, aes(x=treatment, y=co2_ppm)) +
  facet_wrap(~date_format, nrow = 1, ncol = 5)+
                     geom_boxplot(aes(color=treatment), outlier.shape = NA) +
                     geom_point(aes(color=treatment,  shape=treatment), size = 2, position = position_jitterdodge()) +
  scale_color_manual(values=c("#997700","#6699CC","#004488"), labels = c("Dry", "Interim", "Wet"), name="Treatment")+
  ...
  theme(rect=element_rect(fill="transparent"))+
  theme(plot.background = element_rect(color=NA))+
  stat_pvalue_manual(data = CO2_MultComp.DF, comparisons = list(c("D", "I"), c("D", "W"), c("I", "W")), label = "p.SDCF.signif", size = 10, vjust = 1, hide.ns = "p", y.position = max(select_fluxes_combined_w_T0$co2_ppm), step.group.by = "date_format", step.increase = 0.1)
...
```{r N2O T30 ppm Treatment}
# Kruskal-Wallis test on Treatment, and multiple comparisons:
N2O_kruskal<- compare_means(n2o_ppm~treatment, select_fluxes_combined_w_T0, group.by="date_format", method = "kruskal.test")
N2O_multipleComparisons<- compare_means(n2o_ppm~treatment, select_fluxes_combined_w_T0, group.by="date_format", method = "wilcox.test", p.adjust.method = "fdr")
...
# Plotting:
plot.N2O.T30ppm.Trt <- ggplot(select_fluxes_combined_w_T0, aes(x=treatment, y=n2o_ppm)) +
  facet_wrap(~date_format, nrow = 1, ncol = 5)+
                     geom_boxplot(aes(color=treatment), outlier.shape = NA) +
                    geom_point(aes(color=treatment,  shape=treatment), size = 2, position = position_jitterdodge()) +
  scale_color_manual(values=c("#997700","#6699CC","#004488"), labels = c("Dry", "Interim", "Wet"), name="Treatment")+
  ...
  theme(rect=element_rect(fill="transparent"))+
  theme(plot.background = element_rect(color=NA))+
  stat_pvalue_manual(data = N2O_MultComp.DF, comparisons = list(c("D", "I"), c("D", "W"), c("I", "W")), label = "p.SDCF.signif", size = 10, vjust = 1, hide.ns = T, y.position = max(select_fluxes_combined_w_T0$n2o_ppm), step.group.by = "date_format", step.increase = 0.1)
