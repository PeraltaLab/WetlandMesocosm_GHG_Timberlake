...
# These should go after adding T1-T4 average
...
# Copy ghg, then add on averages (because dimensions match)
ghg_avg <- ghg
...
t.test(ghg_ppm_filtered$c2o_ppm, ghg_ppm_filtered$co2_ppm_avg, alternative = "two.sided", mu = 0)
...
t.test(ghg_ppm_filtered$c2o_ppm, ghg_ppm_filtered$co2_ppm_avg, alternative = "two.sided", mu = 0, paired = T)

t.test(ghg_ppm_filtered$n2o_ppm, ghg_ppm_filtered$n2o_ppm_avg, alternative = "two.sided", mu = 0, paired = T)

# Paired T test of plant vs no plant, compare results of T1 only to averaged (non of the inferences would be changed):
...
t.test(c2o_ppm ~ plant, data = ghg_ppm_filtered, alternative = "two.sided", mu = 0, paired = T)
...
get_anova_table(res.aov.CH4_AVG) # Averaging does not change inference here (non-significant for both)
  ...
  # CO2
res.aov.CO2 <- anova_test(data = ghg_ppm_filtered, dv = c2o_ppm, wid = chamber, within = date_short)
get_anova_table(res.aov.CO2) # Sig. different within time points

# Pair-wise comparisons (PWC):
pwc_CO2 <- ghg_ppm_filtered %>%
  pairwise_t_test(
    c2o_ppm ~ date_short, paired = TRUE,
    p.adjust.method = "fdr"
    )
pwc_CO2
...
fourwayAOV_CO2 <- anova_test(
  data = ghg_ppm_filtered, dv = c2o_ppm, wid = chamber,
  between = c(plant, treatment, history),
  within = date_short
  )
