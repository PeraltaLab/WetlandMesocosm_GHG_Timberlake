...
plot.CH4.T30ppm.NPvP <- ggplot(ghg_ppm_filtered, aes(x=plant, y=ch4_ppm)) +
  ...
  theme(text=element_text(size=14),axis.title=element_text(size=14,face="bold"),
          axis.text=element_text(size=14),  
          axis.title.y=element_text(margin=margin(r=10)),
          axis.text.x = element_text(angle = 0),panel.border = element_rect(colour = "black",size=1),strip.text = element_text(size = 14), legend.position="right", legend.title = element_text(size=14), legend.text=element_text(size=14)) + 
          theme(axis.ticks.length=unit(0.3,"cm")) + 
          labs(x = "", y = expression(paste("CH"[4]," (PPM)")), title="") +
...
```{r Extract and save GHG boxplot legend}
leg_NPvP <- get_legend(plot.N2O.T30ppm.NPvP, position = "right") #Extract legend as a legend Grob
...
CH4_kruskal<- compare_means(ch4_ppm~treatment, ghg_ppm_filtered, group.by="date_short", method = "kruskal.test")
CH4_multipleComparisons<- compare_means(ch4_ppm~treatment, ghg_ppm_filtered, group.by="date_short", method = "wilcox.test", p.adjust.method = "fdr")
...
CH4_MultComp.DF$SDCF_p <- c(SDCFlig_CH4_6.28$p.val, SDCFlig_CH4_7.11$p.val, SDCFlig_CH4_7.25$p.val, SDCFlig_CH4_8.11$p.val)
...
plot.CH4.T30ppm.Trt <- ggplot(ghg_ppm_filtered, aes(x=treatment, y=ch4_ppm)) +
  facet_wrap(~date_short, nrow = 1, ncol = 4)+
                     geom_boxplot(aes(color=treatment), outlier.shape = NA) +
                     geom_point(aes(color=treatment), size = 1.5, position = position_jitterdodge()) +
  scale_color_manual(values=c("#997700","#6699CC","#004488"), labels = c("Dry (D)", "Interim (I)", "Wet (W)"), name="Treatment")+
  theme_bw() +
  #Remove plot grid lines
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
  #Set axis title and text properties, tick marks, and labels
  ...
  theme(rect=element_rect(fill="transparent"))+
  theme(plot.background = element_rect(color=NA))+
  #stat_pvalue_manual(data = CH4_multipleComparisons, comparisons = list(c("D", "I"), c("D", "W"), c("I", "W")), label = "p.adj.signif", size = 7, vjust = 1, hide.ns = T, y.position = (max(ghg_ppm_filtered$ch4_ppm)+200), step.group.by = "date_short", step.increase = 0.1)
  stat_pvalue_manual(data = CH4_MultComp.DF, comparisons = list(c("D", "I"), c("D", "W"), c("I", "W")), label = "p.SDCF.signif", size = 10, vjust = 1, hide.ns = T, y.position = (max(ghg_ppm_filtered$ch4_ppm)+200), step.group.by = "date_short", step.increase = 0.1)
...
# Kruskal-Wallis test on Treatment, and multiple comparisons:
CO2_kruskal<- compare_means(co2_ppm~treatment, ghg_ppm_filtered, group.by="date_short", method = "kruskal.test")
CO2_multipleComparisons<- compare_means(co2_ppm~treatment, ghg_ppm_filtered, group.by="date_short", method = "wilcox.test", p.adjust.method = "fdr")
...
CO2_MultComp.DF$SDCF_p <- c(1, 1, 1, SDCFlig_CO2_7.11$p.val, 1, 1, 1, SDCFlig_CO2_8.11$p.val)
...
plot.CO2.T30ppm.Trt <- ggplot(ghg_ppm_filtered, aes(x=treatment, y=co2_ppm)) +
  ...
  theme(rect=element_rect(fill="transparent"))+
  theme(plot.background = element_rect(color=NA))+
  stat_pvalue_manual(data = CO2_MultComp.DF, comparisons = list(c("D", "I"), c("D", "W"), c("I", "W")), label = "p.SDCF.signif", size = 10, vjust = 1, hide.ns = T, y.position = max(ghg_ppm_filtered$co2_ppm), step.group.by = "date_short", step.increase = 0.1)
...
# Kruskal-Wallis test on Treatment, and multiple comparisons:
N2O_kruskal<- compare_means(n2o_ppm~treatment, ghg_ppm_filtered, group.by="date_short", method = "kruskal.test")
N2O_multipleComparisons<- compare_means(n2o_ppm~treatment, ghg_ppm_filtered, group.by="date_short", method = "wilcox.test", p.adjust.method = "fdr")
...
# Add on SDCFlig p-values. If Kruskal was non-significant, input 3 1s, which will be converted to ns:
N2O_MultComp.DF$SDCF_p <- c(SDCFlig_N2O_6.28$p.val, 1,1,1,1,1,1,1,1,1)
...
plot.N2O.T30ppm.Trt <- ggplot(ghg_ppm_filtered, aes(x=treatment, y=n2o_ppm)) +
  ...
  theme(text=element_text(size=14),axis.title=element_text(size=14,face="bold"),
          ...
          axis.text.x = element_text(angle = 0),panel.border = element_rect(colour = "black",size=1),strip.text = element_text(size = 14), legend.position="right", legend.title = element_text(size=14), legend.text=element_text(size=14)) + 
          theme(axis.ticks.length=unit(0.3,"cm")) + 
          labs(x = "Treatment", y = expression(paste("N"[2],"O (PPM)")), title="") +
  #scale_x_discrete(labels=c())+
  theme(rect=element_rect(fill="transparent"))+
  theme(plot.background = element_rect(color=NA))+
  stat_pvalue_manual(data = N2O_MultComp.DF, comparisons = list(c("D", "I"), c("D", "W"), c("I", "W")), label = "p.SDCF.signif", size = 10, vjust = 1, hide.ns = T, y.position = max(ghg_ppm_filtered$n2o_ppm), step.group.by = "date_short", step.increase = 0.1)
 
plot.N2O.T30ppm.Trt
```

```{r Extract and save GHG boxplot legend}
leg_treat <- get_legend(plot.N2O.T30ppm.Trt, position = "right") #Extract legend as a legend Grob
