...
## GHG flux plots for FeS manuscript (AEM00478-25)
```{r CH4}
...
```{r wrangling}
# Add design/meta info onto meta.env_GHG
soil.END.ghg <- meta.env_GHG %>%
  cbind(soil.finish_FULL[,3:5])

# Change history, treatment, and plant to factors:
soil.END.ghg$history <- as.factor(soil.END.ghg$history)
soil.END.ghg$treatment <- as.factor(soil.END.ghg$treatment)
soil.END.ghg$plant <- as.factor(soil.END.ghg$plant)

# Format redox.percent.paint.removed as percent:
soil.END.ghg$redox.percent.formatted <- soil.END.ghg$redox.percent.paint.removed * 100
```


```{r CH4 redox regression}
ch4.lm <- lm(f0_CH4 ~ redox.percent.formatted, data=soil.END.ghg)
summary(ch4.lm)

ch4.redox.regress.plot <- ggplot(soil.END.ghg)  +
  geom_point(aes(x=redox.percent.formatted, y=f0_CH4, color=plant, shape = plant), size=5)+ 
  scale_color_manual(values=c("#E69F00","#009E73"), labels = c("No Plant", "Plant"), name="")+
  scale_fill_manual(values=c("#E69F00","#009E73")) +
  scale_shape_manual(values=c(19, 17), labels = c("No Plant", "Plant"), name="")+
  geom_smooth(aes(x=redox.percent.formatted, y=f0_CH4), formula = y ~ x, method=lm, se=T, color="black")+
  stat_poly_eq(aes(x=redox.percent.formatted, y=f0_CH4, label = paste(after_stat(adj.rr.label), after_stat(p.value.label), sep = "*\", \"*")), data = soil.END.ghg, formula = y ~ x, method = "lm", label.y = "top",label.x = "right")+
  theme_bw()  +
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), 
          axis.title.y=element_text(margin=margin(r=10)), 
    ...
    theme(axis.ticks.length=unit(0.3,"cm")) + labs(x = "", y = expression(paste("CH"[4]," (add flux label)"))) + 
    theme(legend.text=element_text(size=14), legend.title = element_text(size=14), 
...
co2.lm <- lm(f0_CO2 ~ redox.percent.formatted, data=soil.END.ghg)
summary(co2.lm)

co2.redox.regress.plot <- ggplot(soil.END.ghg)  +
  geom_point(aes(x=redox.percent.formatted, y=f0_CO2, color=plant, shape = plant), size=5)+ 
  scale_color_manual(values=c("#E69F00","#009E73"), labels = c("No Plant", "Plant"), name="")+
  scale_fill_manual(values=c("#E69F00","#009E73")) +
  scale_shape_manual(values=c(19, 17), labels = c("No Plant", "Plant"), name="")+
  geom_smooth(aes(x=redox.percent.formatted, y=f0_CO2), formula = y ~ x, method=lm, se=T, color="black")+
  stat_poly_eq(aes(x=redox.percent.formatted, y=f0_CO2, label = paste(after_stat(adj.rr.label), after_stat(p.value.label), sep = "*\", \"*")), data = soil.END.ghg, formula = y ~ x, method = "lm", label.y = "top",label.x = "right")+
  theme_bw()  +
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), 
          axis.title.y=element_text(margin=margin(r=10)), 
          axis.title.x=element_text(margin=margin(t=10))) +
    theme(axis.title=element_text(vjust=1,size=18),
          axis.text=element_text(size=14), axis.text.x = element_text(size=14), panel.border =
          element_rect(colour = "black",size=1)) + 
    theme(axis.ticks.length=unit(0.3,"cm")) + labs(x = "", y = expression(paste("CO"[2]," (add flux label)"))) + 
    theme(legend.text=element_text(size=14), legend.title = element_text(size=14), 
          legend.position = "right")
...
n2o.lm <- lm(f0_N2O ~ redox.percent.formatted, data=soil.END.ghg)
summary(n2o.lm)

n2o.redox.regress.plot <- ggplot(soil.END.ghg)  +
  geom_point(aes(x=redox.percent.formatted, y=f0_N2O, color=plant, shape = plant), size=5)+ 
  scale_color_manual(values=c("#E69F00","#009E73"), labels = c("No Plant", "Plant"), name="")+
  scale_fill_manual(values=c("#E69F00","#009E73")) +
  scale_shape_manual(values=c(19, 17), labels = c("No Plant", "Plant"), name="")+
  geom_smooth(aes(x=redox.percent.formatted, y=f0_N2O), formula = y ~ x, method=lm, se=T, color="black")+
  stat_poly_eq(aes(x=redox.percent.formatted, y=f0_N2O, label = paste(after_stat(adj.rr.label), after_stat(p.value.label), sep = "*\", \"*")), data = soil.END.ghg, formula = y ~ x, method = "lm", label.y = "top",label.x = "right")+
  theme_bw()  +
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), 
          axis.title.y=element_text(margin=margin(r=10)), 
          axis.title.x=element_text(margin=margin(t=10))) +
    theme(axis.title=element_text(vjust=1,size=18),
          axis.text=element_text(size=14), axis.text.x = element_text(size=14), panel.border =
          element_rect(colour = "black",size=1)) + 
    theme(axis.ticks.length=unit(0.3,"cm")) + labs(x = "", y = expression(paste("N"[2],"O (add flux label)"))) + 
    theme(legend.text=element_text(size=14), legend.title = element_text(size=14), 
          legend.position = "right")
...
leg_NPvP_reg <- get_legend(.n2o.redox.regress.plot, position = "bottom") #Extract legend as a legend Grob
...
.GHG_redox_3panel <- ggarrange(.ch4.redox.regress.plot, .co2.redox.regress.plot, .n2o.redox.regress.plot, labels = c("(a)", "(b)", "(c)"), ncol = 1, nrow = 3, legend = "none", common.legend = T)

.GHG_redox_3panel

#ggsave("../figures/pub/_redox_regres.tiff", plot=.GHG_redox_3panel, device="tiff", path=NULL, scale=1, width=10, height=10, dpi=900, limitsize=TRUE, bg="white")
```
