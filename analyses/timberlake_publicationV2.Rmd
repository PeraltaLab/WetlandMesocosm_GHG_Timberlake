...
plot16S_PCoA <- ggplot(pcoa.cent.dataframe.trts, aes(x=V1, y=V2, shape=history_plant, colour=trt)) + geom_errorbarh(aes(xmax=V1+V1e, xmin=V1-V1e, height=0.01), colour="black") +    
  ...
  #Label soil vectors 
  geom_text(data=vec,aes(x=Dim1-0.0125,y=Dim2+0.011,label=parm),size=5, inherit.aes=F)+
  ggtitle("") + 
...
#ggsave("../figures/NC_ASM_2022/16S_PCoA.tiff", plot=plot16S_PCoA, device="tiff", path=NULL, scale=1, width=7, height=5, dpi=300, limitsize=TRUE)

#ggsave("../figures/NC_ASM_2022/16S_PCoA.png", plot=plot16S_PCoA, device="png", path=NULL, scale=1, width=7, height=5, dpi=300, limitsize=TRUE)
...
#ggsave("../figures/pub/iris_tubes.tiff", plot=iris_tubes.Combined, device="tiff", path=NULL, scale=1, width=6, height=5, dpi=900, limitsize=TRUE, bg="transparent")
...
plot.CH4.plant.autoScale <- ggplot(ghg.plant, aes(x=date, y=mg.CH4.N.m.2)) +
                     geom_boxplot(aes(color=treatment), outlier.shape = NA) +
                     geom_point(aes(color=treatment), size = 1, position = position_jitterdodge()) +
  scale_color_manual(values=c("#997700","#6699CC","#004488"), labels = c("Dry", "Interim", "Wet"), name="Treatment")+
  theme_bw() +
  #Remove plot grid lines
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
  ...
  theme(text=element_text(size=14),axis.title=element_text(size=14,face="bold"),
          ...
          axis.title.y=element_text(margin=margin(r=10)),
          panel.border = element_rect(colour = "black",size=1),strip.text = element_text(size = 14), legend.position="none", legend.title = element_text(size=14), legend.text=element_text(size=14)) + 
          theme(axis.ticks.length=unit(0.3,"cm")) + 
...
subvp <- viewport(width = 0.35, height = 0.3, x = 0.67, y = 0.34)
...
CH4.Combined
print(plot.CH4.plant.autoScale, vp=subvp)
```
...
#Panel together using ggarrange{ggpubr}:
CO2.Combined <- ggarrange(plot.CO2.noplant, plot.CO2.plant, labels = c("No Plant", "Plant"), ncol = 1, nrow = 2, legend = "right", common.legend = T)

CO2.Combined

#ggsave("../figures/pub/CO2_BoxPlot.tiff", plot=CO2.Combined, device="tiff", path=NULL, scale=1, width=7, height=6, dpi=900, limitsize=TRUE, bg="transparent")

#ggsave("../figures/pub/CO2_boxplot.jpg", plot=CO2.Combined, device=NULL, path=NULL, scale=1, width=7, height=6, dpi=900, limitsize=TRUE ,bg="transparent")

#ggsave("../figures/pub/CO2_boxplot.pdf", plot=CO2.Combined, device=NULL, path=NULL, scale=1, width=7, height=6, dpi=900, limitsize=TRUE ,bg="transparent")
```
...
# Create a viewport (vp) with appropriate dimensions for inset:
subvp2 <- viewport(width = 0.35, height = 0.3, x = 0.67, y = 0.34)

# This works, but only if ran in console, one line followed by the next. To save the image, must manually click through "Export" on "Plots" tab.
N2O.Combined
print(plot.N2O.plant.autoScale, vp=subvp2)
```
...
#Colin: set.seed? increase permutations to 10000.
