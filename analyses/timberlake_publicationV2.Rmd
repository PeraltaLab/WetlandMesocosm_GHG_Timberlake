...
author: "Regina B. Bledsoe, Ariane L. Peralta, Colin G. Finlay"
...
#ggsave("../figures/pub/16S_PCoA.tiff", plot=plot16S_PCoA, device="tiff", path=NULL, scale=1, width=7, height=5, dpi=300, limitsize=TRUE)

#ggsave("../figures/pub/16S_PCoA.png", plot=plot16S_PCoA, device="png", path=NULL, scale=1, width=7, height=5, dpi=300, limitsize=TRUE)
...
# This works, but only if ran in console, one line followed by the next. To save the image, must manually click through "Export" on "Plots" tab.
#CH4.Combined
...
dbplsr.n2o <- dbplsr(mg.N2O.N.m.2 ~ as.matrix(otu.2.rel.dist), data=soil.ghg , ncomp=31, method="GCV")
summary(dbplsr.n2o)
...
ch4.lm2 <- lm(log(mg.CH4.N.m.2) ~ redox.percent.paint.removed,data=soil.ghg)
anova(ch4.lm2)
summary(ch4.lm2)

#Outlier excluded, improves R2 slightly:
ch4.lm <- lm(log(mg.CH4.N.m.2) ~ redox.percent.paint.removed,data=soil.ghg[-c(17),])
anova(ch4.lm)
summary(ch4.lm)


ch4.redox.regress.plot <- ggplot(soil.ghg[-c(17),])  +
