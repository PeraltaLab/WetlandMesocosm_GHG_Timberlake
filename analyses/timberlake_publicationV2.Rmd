...
metaotu.ad <- adonis(metaotu[,-c(1:9)] ~ plant * history + plant * treatment, method = "bray", data = metaotu, perm=10000, set.seed=42)
metaotu.ad 
```
...
#Plotting:
plot1b <- ggplot(pcoa.cent.dataframe.trts, aes(x=V1, y=V2, shape=history_plant, colour=trt)) + geom_errorbarh(aes(xmax=V1+V1e, xmin=V1-V1e, height=0.01), colour="black") +    
...
Colin: Return to delete this chunk later
```{r Bacteria Indicator Species, Gina's old broken code}
...
#COLIN: try making new.data$plant as.factor

design.type <-new.data$plant
#design.type <- droplevels(design.type) #Error in UseMethod("droplevels") : no applicable method for 'droplevels' applied to an object of class "character"

# <- new.data$treatment # Colin: try grouping by treatment (advice from AP)
...
Colin: trying to copy, paste, and modify 2018_WRC_SoilTeaDecomp.Rmd indval code:
...
```{r Old iris_tube plot, to delete later}
# IRIS tube plot:
iris_tubes <- ggplot(soil_nofield, aes(x=treatment, y=(redox.percent.paint.removed*100), fill=factor(plant), color=treatment)) + 
  geom_boxplot() +
  facet_grid(~ fct_rev(time)) +
  scale_color_manual(values=c("#997700","#6699CC","#004488"), name="Treatment") +
  scale_fill_manual(values=c("white", "darkgreen"), labels=c("No Plant", "Plant"))   +
  geom_point(aes(color=factor(treatment)), size=2, position = position_jitterdodge()) + 
#Set plot elements
theme_bw()  +
...
theme(text=element_text(size=14),axis.title=element_text(size=14,face="bold"),
          axis.text=element_text(size=14),  
          axis.title.y=element_text(margin=margin(r=10)),
          panel.border = element_rect(colour = "black",size=1),strip.text = element_text(size = 14), legend.position="right", legend.title = element_blank(), legend.text=element_text(size=14)) + 
          theme(axis.ticks.length=unit(0.3,"cm")) + 
          labs(x = "Treatment", y = "IRIS Percent", title="") +
  theme(rect=element_rect(fill="transparent"))+
  theme(plot.background = element_rect(color=NA))
...
Colin: Attempting new GHG boxplot:
```{r CH4 plot CF}
...
#My conclusion is that hydrology is the main factor and season is secondary factor in influencing methane rates within the wetland
