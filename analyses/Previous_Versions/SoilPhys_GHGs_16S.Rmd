...
ch4_flux.results <- gasfluxes(ch4_flux_input, method = c("linear","robust linear", "HMR"), plot = F)

co2_flux.results <- gasfluxes(co2_flux_input, method = c("linear","robust linear", "HMR"), plot = F)

n2o_flux.results <- gasfluxes(n2o_flux_input, method = c("linear","robust linear", "HMR"), plot = F)
```
...
res.aov.CH4_AVG <- anova_test(data = ghg_ppm_filtered, dv = ch4_ppm_avg, wid = chamber, within = date_short)
get_anova_table(res.aov.CH4_AVG) # Averaging does not change inference here (non-significant for both)
...
res.aov.CO2_AVG <- anova_test(data = ghg_ppm_filtered, dv = co2_ppm_avg, wid = chamber, within = date_short)
get_anova_table(res.aov.CO2_AVG) # Not sig. different within time points. Averaging DOES change inference.

# Pair-wise comparisons (PWC):
pwc_CO2_AVG <- ghg_ppm_filtered %>%
  pairwise_t_test(
    co2_ppm_avg ~ date_short, paired = TRUE,
    p.adjust.method = "fdr"
    )
pwc_CO2_AVG #Also identifies 7/11 to 7/25 differences (as with non-averaged), but additionally identifies 7/11 to 8/11 difference

# N2O:
res.aov.N2O <- anova_test(data = ghg_ppm_filtered, dv = n2o_ppm, wid = chamber, within = date_short)
get_anova_table(res.aov.N2O)

res.aov.N2O_AVG <- anova_test(data = ghg_ppm_filtered, dv = n2o_ppm_avg, wid = chamber, within = date_short)
get_anova_table(res.aov.N2O_AVG) # Averaging does not change inference here (non-significant for both)
```

```{r Independent Measures ANOVA and Mixed ANOVA}
# CH4:
  # Gina Method (between-subjects factors only is an Independent Measures ANOVA(IMANOVA))
IMANOVA_CH4_Gina <- anova_test(
  data = ghg, dv = mg.CH4.N.m.2, wid = chamber,
  between = c(plant, treatment))
get_anova_table(IMANOVA_CH4_Gina)
  # would want to later do Tukey on treatment

    # Gina Method with date (with date is a Mixed, or split-plot ANOVA(MixANOVA))
MixANOVA_CH4_Gina <- anova_test(
  data = ghg, dv = mg.CH4.N.m.2, wid = chamber,
  between = c(plant, treatment),
  within = date_short)
get_anova_table(MixANOVA_CH4_Gina)
 
 # PPM:
IMANOVA_CH4_ppm <- anova_test(
  data = ghg_ppm_filtered, dv = ch4_ppm, wid = chamber,
  between = c(plant, treatment))
get_anova_table(IMANOVA_CH4_ppm)
  # would want to later do Tukey on treatment
...
get_anova_table(MixANOVA_CH4_ppm)
  ...
  # Average PPM
IMANOVA_CH4_Avg <- anova_test(
  data = ghg_ppm_filtered, dv = ch4_ppm_avg, wid = chamber,
  between = c(plant, treatment))
get_anova_table(IMANOVA_CH4_Avg)
  # would want to later do Tukey on treatment
    
  # Average PPM with date
MixANOVA_CH4_Avg <- anova_test(
  data = ghg_ppm_filtered, dv = ch4_ppm_avg, wid = chamber,
  between = c(plant, treatment),
  within = date_short)
get_anova_table(MixANOVA_CH4_Avg)

  # Flux
IMANOVA_CH4_Flux <- anova_test(
  data = select_fluxes_combined, dv = f0_CH4, wid = chamber,
  between = c(plant, treatment))
get_anova_table(IMANOVA_CH4_Flux)
...
# CO2:
  # Gina Method (between-subjects factors only is an Independent Measures ANOVA(IMANOVA))
IMANOVA_CO2_Gina <- anova_test(
  data = ghg, dv = mg.CO2.N.m.2, wid = chamber,
  between = c(plant, treatment))
get_anova_table(IMANOVA_CO2_Gina)
  # would want to later do Tukey on treatment and plant:treatment

    # Gina Method with date (with date is a Mixed, or split-plot ANOVA(MixANOVA))
MixANOVA_CO2_Gina <- anova_test(
  data = ghg, dv = mg.CO2.N.m.2, wid = chamber,
  between = c(plant, treatment),
  within = date_short)
get_anova_table(MixANOVA_CO2_Gina)
  # would later do Tukey on all except 'plant' 

 # PPM:
IMANOVA_CO2_ppm <- anova_test(
  data = ghg_ppm_filtered, dv = co2_ppm, wid = chamber,
  between = c(plant, treatment))
get_anova_table(IMANOVA_CO2_ppm)
  # would want to later do Tukey on treatment and plant:treatment
...
get_anova_table(MixANOVA_CO2_ppm)
  # Bizarre differences in results from Gina's method, will need to look at this further

  # Average PPM
IMANOVA_CO2_Avg <- anova_test(
  data = ghg_ppm_filtered, dv = co2_ppm_avg, wid = chamber,
  between = c(plant, treatment))
get_anova_table(IMANOVA_CO2_Avg)
  # would want to later do Tukey on treatment and plant:treatment
    
  # Average PPM with date
MixANOVA_CO2_Avg <- anova_test(
  data = ghg_ppm_filtered, dv = co2_ppm_avg, wid = chamber,
  between = c(plant, treatment),
  within = date_short)
get_anova_table(MixANOVA_CO2_Avg)
  # would later do Tukey on all except 'plant'

  # Flux
IMANOVA_CO2_Flux <- anova_test(
  data = select_fluxes_combined, dv = f0_CO2, wid = chamber,
  between = c(plant, treatment))
get_anova_table(IMANOVA_CO2_Flux)
...
# N2O:
  # Gina Method (between-subjects factors only is an Independent Measures ANOVA(IMANOVA))
IMANOVA_N2O_Gina <- anova_test(
  data = ghg, dv = mg.N2O.N.m.2, wid = chamber,
  between = c(plant, treatment))
get_anova_table(IMANOVA_N2O_Gina)
  # would want to later do Tukey on treatment

    # Gina Method with date (with date is a Mixed, or split-plot ANOVA(MixANOVA))
MixANOVA_N2O_Gina <- anova_test(
  data = ghg, dv = mg.N2O.N.m.2, wid = chamber,
  between = c(plant, treatment),
  within = date_short)
get_anova_table(MixANOVA_N2O_Gina)
 
 # PPM:
IMANOVA_N2O_ppm <- anova_test(
  data = ghg_ppm_filtered, dv = n2o_ppm, wid = chamber,
  between = c(plant, treatment))
get_anova_table(IMANOVA_N2O_ppm)
  # would want to later do Tukey on treatment
...
get_anova_table(MixANOVA_N2O_ppm)
  ...
  # Average PPM
IMANOVA_N2O_Avg <- anova_test(
  data = ghg_ppm_filtered, dv = n2o_ppm_avg, wid = chamber,
  between = c(plant, treatment))
get_anova_table(IMANOVA_N2O_Avg)
  # would want to later do Tukey on treatment
    
  # Average PPM with date
MixANOVA_N2O_Avg <- anova_test(
  data = ghg_ppm_filtered, dv = n2o_ppm_avg, wid = chamber,
  between = c(plant, treatment),
  within = date_short)
get_anova_table(MixANOVA_N2O_Avg)

  # Flux
IMANOVA_N2O_Flux <- anova_test(
  data = select_fluxes_combined, dv = f0_N2O, wid = chamber,
  between = c(plant, treatment))
get_anova_table(IMANOVA_N2O_Flux)
...
```{r ANOVAs for boxplot comparison: CH4}
# CH4:
  # Gina method
TwoWayAOV_CH4_Gina <- anova_test(
  data = ghg, dv = mg.CH4.N.m.2, wid = chamber,
  within = c(plant, treatment))

get_anova_table(TwoWayAOV_CH4_Gina)

    # Tukey on treatment
tukey_TwoWayAOV_CH4_Gina <- tukey_hsd(ghg, mg.CH4.N.m.2 ~ treatment)
tukey_TwoWayAOV_CH4_Gina

  # T30 ppm
TwoWayAOV_CH4_T30ppm <- anova_test(
  data = ghg_ppm_filtered, dv = ch4_ppm, wid = chamber,
  within = c(plant, treatment))

get_anova_table(TwoWayAOV_CH4_T30ppm)

    # Tukey on treatment
tukey_TwoWayAOV_CH4_T30ppm <- tukey_hsd(ghg_ppm_filtered, ch4_ppm ~ treatment)
tukey_TwoWayAOV_CH4_T30ppm

  # T1-T4 Average
TwoWayAOV_CH4_AVG <- anova_test(
  data = ghg_ppm_filtered, dv = ch4_ppm_avg, wid = chamber,
  within = c(plant, treatment))

get_anova_table(TwoWayAOV_CH4_AVG)

    # Tukey on treatment
tukey_TwoWayAOV_CH4_AVG <- tukey_hsd(ghg_ppm_filtered, ch4_ppm_avg ~ treatment)
tukey_TwoWayAOV_CH4_AVG

  # Flux
TwoWayAOV_CH4_Flux <- anova_test(
  data = select_fluxes_combined, dv = f0_CH4, wid = ID,
  within = c(plant, treatment))

get_anova_table(TwoWayAOV_CH4_Flux)

    # add on chamber and see if there's a difference:
select_fluxes_combined$chamber <- ghg$chamber

TwoWayAOV_CH4_Flux_2 <- anova_test(
  data = select_fluxes_combined, dv = f0_CH4, wid = chamber,
  within = c(plant, treatment))

get_anova_table(TwoWayAOV_CH4_Flux_2) # Identical to use of 'ID', but use 'chamber' for sanity
```
```{r ANOVAs for boxplot comparison: CO2}
# CO2:
  # Gina method
TwoWayAOV_CO2_Gina <- anova_test(
  data = ghg, dv = mg.CO2.N.m.2, wid = chamber,
  within = c(plant, treatment))

get_anova_table(TwoWayAOV_CO2_Gina)

    # Tukey on treatment
tukey_TwoWayAOV_CO2_Gina <- tukey_hsd(ghg, mg.CO2.N.m.2 ~ treatment)
tukey_TwoWayAOV_CO2_Gina

    # Tukey on plant:treatment
tukey_TwoWayAOV_CO2_Gina_p_t <- tukey_hsd(ghg, mg.CO2.N.m.2 ~ plant:treatment)
tukey_TwoWayAOV_CO2_Gina_p_t

  # T30 ppm
TwoWayAOV_CO2_T30ppm <- anova_test(
  data = ghg_ppm_filtered, dv = co2_ppm, wid = chamber,
  within = c(plant, treatment))

get_anova_table(TwoWayAOV_CO2_T30ppm)

    # Tukey on treatment
tukey_TwoWayAOV_CO2_T30ppm <- tukey_hsd(ghg_ppm_filtered, co2_ppm ~ treatment)
tukey_TwoWayAOV_CO2_T30ppm

    # Tukey on plant:treatment
tukey_TwoWayAOV_CO2_T30ppm_p_t <- tukey_hsd(ghg_ppm_filtered, co2_ppm ~ plant:treatment)
tukey_TwoWayAOV_CO2_T30ppm_p_t

  # T1-T4 Average
TwoWayAOV_CO2_AVG <- anova_test(
  data = ghg_ppm_filtered, dv = co2_ppm_avg, wid = chamber,
  within = c(plant, treatment))

get_anova_table(TwoWayAOV_CO2_AVG)

    # Tukey on treatment
tukey_TwoWayAOV_CO2_AVG <- tukey_hsd(ghg_ppm_filtered, co2_ppm_avg ~ treatment)
tukey_TwoWayAOV_CO2_AVG

    # Tukey on plant:treatment
tukey_TwoWayAOV_CO2_AVG_p_t <- tukey_hsd(ghg_ppm_filtered, co2_ppm_avg ~ plant:treatment)
tukey_TwoWayAOV_CO2_AVG_p_t

  # Flux
TwoWayAOV_CO2_Flux <- anova_test(
  data = select_fluxes_combined, dv = f0_CO2, wid = chamber,
  within = c(plant, treatment))

get_anova_table(TwoWayAOV_CO2_Flux)
```
```{r ANOVAs for boxplot comparison: N2O}
# N2O:
  # Gina method
TwoWayAOV_N2O_Gina <- anova_test(
  data = ghg, dv = mg.N2O.N.m.2, wid = chamber,
  within = c(plant, treatment))

get_anova_table(TwoWayAOV_N2O_Gina)

  # T30 ppm
TwoWayAOV_N2O_T30ppm <- anova_test(
  data = ghg_ppm_filtered, dv = n2o_ppm, wid = chamber,
  within = c(plant, treatment))

get_anova_table(TwoWayAOV_N2O_T30ppm)

  # T1-T4 Average
TwoWayAOV_N2O_AVG <- anova_test(
  data = ghg_ppm_filtered, dv = n2o_ppm_avg, wid = chamber,
  within = c(plant, treatment))

get_anova_table(TwoWayAOV_N2O_AVG)

  # Flux
TwoWayAOV_N2O_Flux <- anova_test(
  data = select_fluxes_combined, dv = f0_N2O, wid = chamber,
  within = c(plant, treatment))

get_anova_table(TwoWayAOV_N2O_Flux)
```
...
leg <- get_legend(plot.N2O.noplant, position = "right") #Extract legend as a legend Grob
...
## GHG plots T1-T4 ppm average

```{r CH4 T30 ppm plot}
plot.CH4.Avg_ppm.noplant <- ggplot(ghg_ppm_filtered.noplant, aes(x=date_short, y=ch4_ppm_avg)) +
                     geom_boxplot(aes(color=treatment), outlier.shape = NA) +
                     geom_point(aes(color=treatment), size = 1.5, position = position_jitterdodge()) +
  scale_color_manual(values=c("#997700","#6699CC","#004488"), labels = c("Dry", "Interim", "Wet"), name="Treatment")+
  theme_bw() +
...
plot.CH4.Avg_ppm.noplant


plot.CH4.Avg_ppm.plant <- ggplot(ghg_ppm_filtered.plant, aes(x=date_short, y=ch4_ppm_avg)) +
                     geom_boxplot(aes(color=treatment), outlier.shape = NA) +
                     geom_point(aes(color=treatment), size = 1.5, position = position_jitterdodge()) +
  scale_color_manual(values=c("#997700","#6699CC","#004488"), labels = c("Dry", "Interim", "Wet"), name="Treatment")+
  theme_bw() +
...
plot.CH4.Avg_ppm.plant

#Panel together using ggarrange{ggpubr}:
#No legend version, for combining figures later:
CH4.Avg_ppm.Combined <- ggarrange(plot.CH4.Avg_ppm.noplant, plot.CH4.Avg_ppm.plant, labels = c("No Plant", "Plant"), ncol = 1, nrow = 2, legend = "none")

CH4.Avg_ppm.Combined

# Add an inset to show small values for the 'plant' treatment:
# First, render plot with no axes labels and smaller axes text:
plot.CH4.Avg_ppm.plant.autoScale <- ggplot(ghg_ppm_filtered.plant, aes(x=date_short, y=ch4_ppm_avg)) +
                     geom_boxplot(aes(color=treatment), outlier.shape = NA, lwd = 2) +
                     geom_point(aes(color=treatment), size = 3.5, position = position_jitterdodge()) +
  scale_color_manual(values=c("#997700","#6699CC","#004488"), labels = c("Dry", "Interim", "Wet"), name="Treatment")+
  theme_bw() +
...
plot.CH4.Avg_ppm.plant.autoScale

#ggsave("../figures/pub/Fig3/Avg_ppm/CH4_Inset_Avg_ppm.tiff", plot=plot.CH4.Avg_ppm.plant.autoScale, device="tiff", path=NULL, scale=1, width=7, height=5, dpi=300, limitsize=TRUE, bg="white")
...
```{r CO2 T30 ppm plot}
plot.CO2.Avg_ppm.noplant <- ggplot(ghg_ppm_filtered.noplant, aes(x=date_short, y=co2_ppm_avg)) +
                     geom_boxplot(aes(color=treatment), outlier.shape = NA) +
                     geom_point(aes(color=treatment), size = 1.5, position = position_jitterdodge()) +
  scale_color_manual(values=c("#997700","#6699CC","#004488"), labels = c("Dry", "Interim", "Wet"), name="Treatment")+
  theme_bw() +
...
plot.CO2.Avg_ppm.noplant


plot.CO2.Avg_ppm.plant <- ggplot(ghg_ppm_filtered.plant, aes(x=date_short, y=co2_ppm_avg)) +
                     geom_boxplot(aes(color=treatment), outlier.shape = NA) +
                     geom_point(aes(color=treatment), size = 1.5, position = position_jitterdodge()) +
  scale_color_manual(values=c("#997700","#6699CC","#004488"), labels = c("Dry", "Interim", "Wet"), name="Treatment")+
  theme_bw() +
...
plot.CO2.Avg_ppm.plant

#Panel together using ggarrange{ggpubr}:
#No legend version, for combining figures later:
CO2.Avg_ppm.Combined <- ggarrange(plot.CO2.Avg_ppm.noplant, plot.CO2.Avg_ppm.plant, labels = c("No Plant", "Plant"), ncol = 1, nrow = 2, legend = "none")

CO2.Avg_ppm.Combined

# Average ppm with plants is much lower than without plants. This is likely the result of photosynthesis over the longer timeperiod. Will create an inset, autoscaled version for this:

plot.CO2.Avg_ppm.plant.autoScale <- ggplot(ghg_ppm_filtered.plant, aes(x=date_short, y=co2_ppm_avg)) +
                     geom_boxplot(aes(color=treatment), outlier.shape = NA, lwd = 2) +
                     geom_point(aes(color=treatment), size = 3.5, position = position_jitterdodge()) +
  scale_color_manual(values=c("#997700","#6699CC","#004488"), labels = c("Dry", "Interim", "Wet"), name="Treatment")+
  theme_bw() +
...
plot.CO2.Avg_ppm.plant.autoScale

#ggsave("../figures/pub/Fig3/Avg_ppm/CO2_Inset_Avg_ppm.tiff", plot=plot.CO2.Avg_ppm.plant.autoScale, device="tiff", path=NULL, scale=1, width=7, height=5, dpi=300, limitsize=TRUE, bg="white")
```

```{r N2O T30 ppm Boxplot}
plot.N2O.Avg_ppm.noplant <- ggplot(ghg_ppm_filtered.noplant, aes(x=date_short, y=n2o_ppm_avg)) +
                     geom_boxplot(aes(color=treatment), outlier.shape = NA) +
                     geom_point(aes(color=treatment), size = 1.5, position = position_jitterdodge()) +
  scale_color_manual(values=c("#997700","#6699CC","#004488"), labels = c("Dry", "Interim", "Wet"), name="Treatment")+
  theme_bw() +
...
plot.N2O.Avg_ppm.noplant


plot.N2O.Avg_ppm.plant <- ggplot(ghg_ppm_filtered.plant, aes(x=date_short, y=n2o_ppm_avg)) +
                     geom_boxplot(aes(color=treatment), outlier.shape = NA) +
                     geom_point(aes(color=treatment), size = 1.5, position = position_jitterdodge()) +
  scale_color_manual(values=c("#997700","#6699CC","#004488"), labels = c("Dry", "Interim", "Wet"), name="Treatment")+
  theme_bw() +
...
plot.N2O.Avg_ppm.plant


#Panel together using ggarrange{ggpubr}:
# Version without legend for multi-panel figure:
N2O.Avg_ppm.Combined <- ggarrange(plot.N2O.Avg_ppm.noplant, plot.N2O.Avg_ppm.plant, labels = c("No Plant", "Plant"), ncol = 1, nrow = 2, legend = "none")

N2O.Avg_ppm.Combined

# Add an inset to show small values for the 'plant' treatment:
# First, render plot with no axes labels and smaller axes text:
plot.N2O.Avg_ppm.plant.autoScale <- ggplot(ghg_ppm_filtered.plant, aes(x=date_short, y=n2o_ppm_avg)) +
                     geom_boxplot(aes(color=treatment), outlier.shape = NA, lwd = 2) +
                     geom_point(aes(color=treatment), size = 3.5, position = position_jitterdodge()) +
  scale_color_manual(values=c("#997700","#6699CC","#004488"), labels = c("Dry", "Interim", "Wet"), name="Treatment")+
  theme_bw() +
...
plot.N2O.Avg_ppm.plant.autoScale

#ggsave("../figures/pub/Fig3/Avg_ppm/N2O_Inset_Avg_ppm.tiff", plot=plot.N2O.Avg_ppm.plant.autoScale, device="tiff", path=NULL, scale=1, width=7, height=5, dpi=300, limitsize=TRUE, bg="white")
...
```{r Combined GHG Boxplot}
comb_Avg_ppm_BoxPlot <- ggarrange(CH4.Avg_ppm.Combined, CO2.Avg_ppm.Combined, N2O.Avg_ppm.Combined, labels = c("A", "B", "C"), ncol = 3, nrow = 1, legend = "none")

comb_Avg_ppm_BoxPlot

#ggsave("../figures/pub/Fig3/Avg_ppm/BasePlots_Avg_ppm.tiff", plot=comb_Avg_ppm_BoxPlot, device="tiff", path=NULL, scale=1, width=15, height=6, dpi=600, limitsize=TRUE, bg="white")
```
